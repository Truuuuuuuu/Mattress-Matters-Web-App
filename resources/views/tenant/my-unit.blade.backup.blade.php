                    ...
                    <div class=" lg:flex gap-2 mt-5 ">
                        <div class="flex flex-col justify-center items-center flex-1 mb-3  items-center ">
                            <div class="w-42 lg:w-full p-5 ">
                                <img src="{{asset('images/3D-bhouse-model.svg')}}" alt="3D"
                                     class="cursor-pointer  object-contain transition-transform duration-300 hover:scale-110">
                            </div>
                            <div class="flex flex-col items-center text-center">
                                <p class="text-lg font-semibold line-clamp-1"
                                   title="{{$myUnit->listing->title}}">{{$myUnit->listing->title}}</p>
                                <a href="{{route('listings.show', $myUnit->listing)}}"
                                   class="btn btn-neutral btn-sm px-10">Visit </a>
                            </div>
                        </div>
                        <div class="flex-2 border rounded-xl px-4 py-5 space-y-3 mb-3">
                            <div>
                                <h1 class="text-sm font-bold text-base-content/70 -mb-2">Title</h1>
                                <p title="{{$myUnit->listing->title}}"
                                   class="line-clamp-1">{{$myUnit->listing->title}}</p>
                            </div>
                            <div>
                                <h1 class="text-sm font-bold text-base-content/70 -mb-2">Address</h1>
                                <p title="{{$myUnit->listing->address}}"
                                   class="line-clamp-1">{{$myUnit->listing->address}}</p>
                            </div>
                            <div>
                            ...
                            <div class="flex-1 border rounded-xl px-2 pt-7 pb-3 ">
                                <div>
                                    ...
                                    <h1 class="text-xl font-bold">
                                        ...
                                        @elseif($invoiceInfo['status'] === 'overdue')
                                            <span
                                                class="text-error">{{ $invoiceInfo['due_date']?->format('F d, Y') }}</span>
                                        @else
                                            {{ $invoiceInfo['due_date']?->format('F d, Y') }}
                                        @endif
                                    </h1>
                                    @if($invoiceInfo['status'] === 'no_invoice' || $invoiceInfo['status'] === 'paid')
                                        <p class="text-xs text-base-content/50">Note: Invoice will be generated 7 days
                                            before due date</p>
                                    @endif
                                ...
                                <div class="w-full mt-4">
                                    <a href="{{ route('tenant.soa') }}" class="btn btn-success w-full">Manage
                                        Billing</a>
                                </div>
                            </div>
                            <div class="flex-1 border rounded-xl px-2 py-4 space-y-2">
                                ...
                                <div>
                                    ...
                                    <h1 class="text-3xl font-bold">
                                        ₱{{number_format($myUnit->listing->rent_cost,2)}}</h1>
                                </div>
                            </div>
                            <div class="border rounded-xl px-2 py-4 flex flex-col items-center mb-3">
                                ...
                                <div class="w-full">
                                    ...
                                    <a href="{{route('messages.show', $myUnit->listing->host->user)}}"
                                       class="btn btn-neutral w-full mt-2">Message</a>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div>
            ...
            <div class="w-full">
                ...
                <div class="border border-red-900 rounded-xl px-4 py-3">
                        ...
                        <div>
                            @if ($myUnit->moveOutNotice?->isActive() && $myUnit->moveOutNotice->isCancellable())
                                <button onclick="cancel_move_out_modal.showModal()"
                                        class="btn w-full btn-outline btn-error shrink-0">
                                    Cancel Notice
                                </button>
                            @elseif (!$myUnit->moveOutNotice?->isActive() && (!$myUnit->moveOutNotice || $myUnit->moveOutNotice->canSubmitMoveOut()))
                                <button onclick="confirm_move_out_modal.showModal()"
                                        class="btn btn-outline btn-error shrink-0 w-full">
                                    Move Out
                                </button>
                            @endif
                        </div>
                    ...
                    <div>
                        @if ($myUnit->moveOutNotice?->isActive() && !$myUnit->moveOutNotice->isCancellable())
                            <p class="text-sm text-gray-500 mt-2">Your move out notice can no longer be cancelled.
                                Contact your landlord directly.</p>
                        ...
                        @elseif ($myUnit->moveOutNotice?->isCancelled() && !$myUnit->moveOutNotice->canSubmitMoveOut())
                            <p class="text-sm text-gray-500 mt-2">You recently cancelled a move out notice. Please
                                wait {{$myUnit->moveOutNotice->daysUntilCanResubmit()}} days before submitting a new
                                one.</p>
                        @endif
                    </div>
                </div>
        ...
        @elseif($myUnit?->reservation?->status === 'accepted')
            <div class="w-full flex justify-center items-center py-10">
                <div class="w-full max-w-lg  flex flex-col items-center">
                    ...
                    <div class="w-xs  ">
                        <img src="{{asset('images/3D-bhouse-model.svg')}}" alt="3D"
                             class="cursor-pointer w-full h-auto object-contain transition-transform duration-300 hover:scale-110">
                    </div>
