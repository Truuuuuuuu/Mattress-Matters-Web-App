...
<div class="avatar">
    ...
    @if($user)
        @if(request()->routeIs('profile.index') || request()->routeIs('profile.show'))
            ...
            @else
                <div class="avatar " >
                    <div class="mask mask-squircle h-24 w-24 lg:h-32 lg:w-32 bg-base-300 flex items-center justify-center" style="box-shadow: 0 8px 32px rgba(37, 99, 235, 0.15)">
            ...
            @else
                <a href="{{ $user === auth()->user() ? route('profile.index') : route('profile.show', $user) }}">
                    <div class="avatar  mb-4" >
                        <div {{$attributes->class(["mask mask-squircle h-{$height} w-{$width} bg-base-300 flex items-center justify-center hover:bg-primary/10" ])}} >
