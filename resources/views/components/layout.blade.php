...
<div>
    @if (!isset($hideNavbar))
        ...
        <nav class="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-base-100 border-t border-base-300 flex justify-around items-center h-16">
            ...
            <a href="{{route('message.index', auth()->user()->id)}}" class="flex flex-col items-center text-xs gap-1 text-base-content/70">
            ...
            <div class="w-full max-w-7xl mx-auto flex justify-between items-center py-4 px-5">
                   ...
                   @auth
                       <div class="flex space-x-3 font-bold ">
                           <a href="{{route('message.index', auth()->user()->id)}}"  class="flex btn btn-ghost btn-circle lg:bg-gray-200 lg:text-black" tabindex="0" role="button">
